...
DEFAULT_ROOT = ROOT_DIR / "results" / "bmi_decoding" / "jango"
DEFAULT_OUT = ROOT_DIR / "results" / "bmi_decoding" / "jango.png"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Plot multi-seed Jango Mamba/MotifMamba accuracy bars with seed-level error bars."
    )
    parser.add_argument("--root", type=Path, default=DEFAULT_ROOT)
    parser.add_argument("--out", type=Path, default=DEFAULT_OUT)
    parser.add_argument("--dpi", type=int, default=300)
    parser.add_argument("--ymin", type=float, default=90.0)
...
def read_summary_acc(summary_path: Path) -> float:
    with open(summary_path, "r", encoding="utf-8") as f:
        summary = json.load(f)
    acc = summary.get("mean_best_eval_acc")
    if acc is None:
        raise KeyError(f"{summary_path} does not contain mean_best_eval_acc")
...
def discover_seed_values(root: Path) -> dict[str, list[tuple[int, float]]]:
    ...
    for seed_dir in seed_dirs:
        ...
        for model in MODEL_ORDER:
            summary_path = seed_dir / f"jango_{model}_seed_{seed}" / "summary.json"
...
def write_csvs(values: dict[str, list[tuple[int, float]]], stats: dict[str, dict[str, float]], out_path: Path) -> None:
    ...
    with open(stats_csv, "w", encoding="utf-8", newline="") as f:
        ...
        for model in MODEL_ORDER:
            s = stats[model]
            writer.writerow([DISPLAY_NAME[model], int(s["n"]), f"{s['mean']:.6f}", f"{s['sd']:.6f}", f"{s['var']:.6f}"])

    print(f"Saved {value_csv}")
    print(f"Saved {stats_csv}")


def plot(values: dict[str, list[tuple[int, float]]], stats: dict[str, dict[str, float]], out_path: Path, dpi: int, ymin: float, ymax: float) -> None:
    plt.rcParams.update(
        {
            "font.family": "serif",
...
def main() -> None:
    ...
    plot(values, stats, args.out, dpi=args.dpi, ymin=args.ymin, ymax=args.ymax)
