...
DEFAULT_ROOT = ROOT_DIR / "results" / "bmi_decoding" / "mice_lick"
DEFAULT_OUT = ROOT_DIR / "results" / "bmi_decoding" / "mice_lick.png"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Plot multi-seed mice lick Mamba/MotifMamba accuracy bars with seed-level SD."
    )
    parser.add_argument("--root", type=Path, default=DEFAULT_ROOT)
    parser.add_argument("--out", type=Path, default=DEFAULT_OUT)
    parser.add_argument("--dpi", type=int, default=300)
    parser.add_argument("--ymin", type=float, default=75.0)
    parser.add_argument("--ymax", type=float, default=80.0)
    parser.add_argument("--ytick-step", type=float, default=1.0)
...
def read_summary_acc(summary_path: Path) -> float:
    ...
    raise KeyError(f"{summary_path} does not contain a supported accuracy field")
...
def discover_seed_values(root: Path) -> dict[str, list[tuple[int, float]]]:
    ...
    for seed_dir in seed_dirs:
        ...
        for model in MODEL_ORDER:
            summary_path = seed_dir / f"mice_lick_{model}_seed_{seed}" / "summary.json"
...
def make_yticks(ymin: float, ymax: float, step: float) -> list[float]:
    ticks = []
    current = ymin
...
def plot_bar(
    ...
    ax.set_title("Mice Lick", pad=6)
    ax.set_xlabel("Models", labelpad=5)
    ax.set_ylabel("Accuracy (%)", labelpad=5)
    ax.set_xticks(x)
    ax.set_xticklabels(labels)
    ax.set_ylim(ymin, ymax)
    ax.set_yticks(make_yticks(ymin, ymax, ytick_step))
    ...
    label_offset = max((ymax - ymin) * 0.02, 0.06)
