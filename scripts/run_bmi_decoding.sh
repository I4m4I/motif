...
ROOT_DIR="$(pwd)"
SCRIPTS_DIR="${ROOT_DIR}/training"
RESULTS_ROOT="${ROOT_DIR}/results/bmi_decoding"
RUNS_ROOT="${FIG5_OUTPUT_ROOT:-${RESULTS_ROOT}/runs}"
FIGURES_DIR="${ROOT_DIR}/results/bmi_decoding"
...
if [[ "${RUN_PLOT}" == "1" ]]; then
  "${PYTHON_BIN}" "${ROOT_DIR}/scripts/run_bmi_decoding.py"
else
