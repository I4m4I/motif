...
DEFAULT_OUT_DIR = Path(os.environ.get("FIG5_OUTPUT_ROOT", str(ROOT_DIR / "results" / "bmi_decoding" / "runs"))) / "jango_force"
...
def train_jango_split(
    ...
    from models.motif_mamba import MambaClassifier
...
def run_file_holdout(args: argparse.Namespace):
    from data.datasets import JangoForceDataset, resolve_jango_split, summarize_records
...
def run_daily_8020(args: argparse.Namespace):
    ...
    from data.datasets import JangoForceDataset, resolve_jango_daily_splits, summarize_records
